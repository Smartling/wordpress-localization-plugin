...
class CloneTest extends SmartlingUnitTestCaseAbstract
{
    public function testNoMediaDuplicationOnCloning(): void
    {
        ...
        $rootPostTargetId = $rootSubmission->getTargetId();
        $this->assertEquals('<!-- wp:test/post {"id":' . $childPostTargetId . '} /-->', get_post($rootPostTargetId)->post_content, 'Expected root post to reference child post id at the target blog');
        $this->assertEquals($rootPostTargetId, get_post($childPostTargetId)->post_parent, 'Expected child post\'s parent_id to be equal to it\'s parent post');
