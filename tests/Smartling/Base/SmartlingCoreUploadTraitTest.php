...
HTML;
    ...
    public function testFindSubmissionsInJob()
    {
        $relatedSubmission1 = $this->createMock(SubmissionEntity::class);
        $relatedSubmission1->method('getId')->willReturn(3);
        $relatedSubmission2 = $this->createMock(SubmissionEntity::class);
        $relatedSubmission2->method('getId')->willReturn(5);
        ...
        $db = $this->mockDbAl();
        $db->method('getColumnArray')->willReturn([5]);
        $uploadQueueManager = new UploadQueueManager($db);

        $x = $this->getSmartlingCoreUpload(submissionManager: $submissionManager, uploadQueueManager: $uploadQueueManager);
        $submissions = $x->findSubmissionsInJob([], 'test');
        $this->assertCount(1, $submissions);
        $this->assertEquals($relatedSubmission2, $submissions[0]);
    }
