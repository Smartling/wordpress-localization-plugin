...
class ExternalContentManager
{
    ...
    public function getExternalContent(array $source, SubmissionEntity $submission, bool $raw): array
    {
        foreach ($this->handlers as $handler) {
            if ($handler->canHandle($this->contentTypeHelper, $this->pluginHelper, $this->wpProxy, $submission->getSourceId(), $submission->getContentType())) {
    ...
    public function getExternalRelations(string $contentType, int $id): array
    {
        $result = [];
        foreach ($this->handlers as $handler) {
            if ($handler->canHandle($this->contentTypeHelper, $this->pluginHelper, $this->wpProxy, $id, $contentType)) {
